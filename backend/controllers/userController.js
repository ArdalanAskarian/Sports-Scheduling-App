...
const userHome = async (req, res) => {
  ...
  try {
    ...
    // Check if userTeam and userTeam.games are defined before accessing them
    const allGames = userTeam && userTeam.games ? userTeam.games : [];

    // Calculate team stats
    const teamStats = {
      wins: userTeam ? userTeam.wins : 0,
      losses: userTeam ? userTeam.losses : 0,
      ties: userTeam ? userTeam.ties : 0,
    };
  ...
  try {
    ...
    res.status(200).json({
      user: {
        ...
        email: user.email,
        profileImage: user.profileImage, // Assuming you have a profileImage field in your User model
      },
      success: true,
    });
  } catch (error) {
...
const updateUserProfileImage = async (req, res) => {
  try {
    ...
    // Assuming you have an 'image' field in your request body
